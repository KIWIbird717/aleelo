...
export namespace ChatService {
  ...
  export const postMessage = (options: PostMessageOptions) => {
    return errorHandler(
      thisName("postMessage"),
      serverApi.post<IPostMessageResult>("/game-chat/post-message", options),
    );
  };
  ...
  export const getMessages = (chatId: string, pagination: IPagination) => {
    return errorHandler(
      thisName("getMessages"),
      serverApi.get<Array<IChatMessage>>(`game-chat/list/${chatId}`, { params: pagination }),
    );
  };
  ...
  export const startOnboardingReport = () => {
    return errorHandler(
      thisName("startOnboardingReport"),
      serverApi.put("/game-chat/onboarding-report/start"),
    );
  };
  ...
  export const startPracticeDialog = (gameId: string, cellId: number) => {
    return errorHandler(
      thisName("startPracticeDialog"),
      serverApi.post("/game-chat/start-practice", {
        gameId,
        cellId,
      }),
    );
  ...
  export const sendPracticeMessage = (practiceId: string) => {
    return errorHandler(
      ...
      serverApi.put(`/game-chat/practices/${practiceId}/send-message`),
    );
  };
  ...
  export const startFinalReportDialog = () => {
    return errorHandler(
      thisName("startFinalReportDialog"),
      serverApi.post("/game-chat/start-final-report"),
    );
  };
